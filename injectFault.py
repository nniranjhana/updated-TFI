...
#				if(str(op) == "Ops.ADD" and len(res[0])==10):
#					print(res)
...
def injectFaultUnpack(a):
	...
	resOp = tf.unstack(a, a.shape[1], 1)
	with tf.Session() as sess:
		res = sess.run(resOp)
...
def injectFaultSplit(a, b):
	"Function to call injectFault on Split"
	# FIXME: Implement this functionality
#	import pdb;pdb.set_trace()
        logging.debug("Calling Operator Split " + getArgs(a, b))
	res = np.split(b, b.shape[1]/b.shape[0], a)
#	with tf.Session() as sess:
#		res = resOp.eval()
	return res
